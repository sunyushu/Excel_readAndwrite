...
public class Om_cust_price_listDaoImpl implements Om_cust_price_listDaoI {

	public List<Om_cust_price_list> list(int page, int rows, String sort, String order) throws SQLException {
	...
	public int getTotal() throws SQLException {
		...
		int result = 0;
		try {
			ps = conn.prepareStatement("select count(*) from Om_cust_price_list");
			rs = ps.executeQuery();
			if (rs.next()) {
				result = rs.getInt(1);
			}
